#*********************SE LLAMA A LOS ARCHIVOS Y LIBRERÍAS REQUERIDAS***************
import librerias
import os
import shutil
#**********************************************************************************
...
filename = 'default-log-DP_1.txt'
filename2 = 'default-log-DP_'
respuesta_final =[]
#***********************************************************************************
...
def valida_dir_file(filename,filename_final):
    #se guarda la ruta del archivo y se le concatena el nombre del mismo
    path_archivo_dp1 = librerias.os.path.join(librerias.logsdir, filename)
    path_archivo_dp2 = librerias.os.path.join(librerias.logsdir, filename_final)
    ...
    shutil.copy(filename, librerias.logsdir)
    shutil.copy(filename_final, librerias.logsdir)
...
for i in range(total_targets):
    ...
    #se asigna la url con base en el archivo librerías, que a su vez contiene información de la url a usar
    url= librerias.hosts.ipaddr[i] + librerias.hosts.defaultlog
   #ejecutamos el request y guardamos la respuesta
    ...
    #validamos si la respuesta contiene la clave CPUUsage (por si es virtual no tendrá esta información)
    if 'error' in response.json():
        #convertimos la respuesta en formato json
        parsed_response = response.json()['error']
    else:
        #se extrae el contenido del elemento file
        contenido_codificado=parsed_response['file']
        #se decodifica el contenido de file que viene en b64
        contenido_decodificado=librerias.base64.b64decode(contenido_codificado)

    #generamos el nombre de archivo para cada datapower
    filename_final =f"{filename2}{i+1}.txt"
    ...
    if response.status_code == 200:
        ...
        if i==0:
            ...
            with open(filename, 'wb') as a:
                a.write(contenido_decodificado)          
            ...
            with open(filename_final, 'wb') as a:
                a.write(contenido_decodificado)                     
