...
for i in range(total_targets):
    ...
    url = librerias.hosts.ipaddr[i] + librerias.hosts.defaultstat + 'MemoryStatus'
    #ejecutamos el request y guardamos la respuesta
    response = librerias.requests.get(url,headers=headers, verify=False)

    if 'MemoryStatus' in response.json():
        #convertimos la respuesta en formato json
        parsed_response = response.json()['MemoryStatus']
    else:
         #convertimos la respuesta en formato json con el mensaje que devolvió
