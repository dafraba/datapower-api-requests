...
filename = 'filesystem_DP1.json'
filename2 = 'filesystem_DP'
respuesta_final =[]
#***********************************************************************************
...
for i in range(total_targets):
    ...
    url = librerias.hosts.ipaddr[i] + librerias.hosts.defaultstat + 'FilesystemStatus'
    #ejecutamos el request y guardamos la respuesta
    response = librerias.requests.get(url,headers=headers, verify=False)

    #convertimos la respuesta en formato json
    parsed_response = response.json()['FilesystemStatus'] 
